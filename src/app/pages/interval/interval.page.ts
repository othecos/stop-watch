...
export class IntervalPage implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    ...
    const exerciseSubscription = this.exercisesService.eventEmitter.subscribe((event: {
    ...
    const clockerSubscription = this.clockerService.eventsEmitter.subscribe(async (event) => {
      if (event === 'dancing') {
        this.playFireworks = true;
        setTimeout(() => { this.playFireworks = false; }, 6000);
      } else {
        this.updateButtonController();
      }
    });
  ...
  public async onPlay() {
    ...
    const currentElementIndex = await this.getCurrentExerciseIndex();
  ...
  public async onRefresh() {
    ...
    const refreshedExercises = await this.clockerService.refresh(this.exercisesService.exercises);
  ...
  public getAnimationClasses(value: Exercise) {
    const classes = [];
    let currentExercise = value;
    if (!value) {
      currentExercise = this.exercisesService.exercises.find((exercise) => exercise.progress.initiated && !exercise.progress.finished);
    }
    if (this.enableDancing) {
      classes.push('dancing-side');
    }

    if (this.clockerService.intervalTimer.isRunning && currentExercise.counter <= 5) {
      classes.push('bounce');
    }
  ...
  private async updateSlides(counter = 0) {
    if (this.slides) {
      ...
      return this.updateSlides(counter + 1);
    } else {
      return false;
    }
  }
  ...
  private async fixIonBug() {
    if (this.exercisesService.exercises.length > 2) {
      const ionSlide = document.body.getElementsByTagName('ion-slide');
  ...
  private async getCurrentExerciseIndex() {
    return await this.exercisesService.exercises.findIndex((exercise) => exercise.progress.initiated && !exercise.progress.finished);
  }
  private async cleanCurrentExercise() {
    const currentElementIndex = await this.getCurrentExerciseIndex();
    if (currentElementIndex !== -1) {
      const currentExercise = this.exercisesService.exercises[currentElementIndex];
      await this.clockerService.stop(currentExercise);
      this.exercisesService.exercises[currentElementIndex] = await this.clockerService.rebuild(currentExercise);
    }
  }
  ...
  private async updateButtonController() {
    ...
    // Restart
    this.buttonsController.restart.disabled = !(
      this.clockerService.intervalTimer.isFinished ||
      this.clockerService.intervalTimer.isRunning);
  ...
  private async lockSlides(counter = 0) {
    if (this.slides) {
      ...
      await Utils.sleep(200);
      return this.lockSlides(counter + 1);
    } else {
      return false;
    }
  }
  ...
  isDelayRunning(counter) {
    return this.clockerService.intervalTimer.stages.delay.isInitiated &&
      this.clockerService.intervalTimer.stages.delay.isRunning &&
      !this.clockerService.intervalTimer.stages.delay.isFinished &&
      counter > 5;
  }
}
